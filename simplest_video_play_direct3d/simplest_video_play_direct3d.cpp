...
#define LIBCRLIVE_EXPORT_API
#ifdef LIBCRLIVE_EXPORT_API
...
int InitD3D(HWND hwnd, unsigned long lWidth, unsigned long lHeight)
{
	...
	lRet = m_pDirect3DDevice->CreateRenderTarget(lWidth, lHeight,
		(D3DFORMAT)MAKEFOURCC('Y', 'V', '1', '2'),
...
extern "C" EXPORT_API bool Render()
{
	HRESULT lRet;
	//Read Data
	//RGB
	...
	for (i = 0; i < pixel_h / 2; i++) {
		memcpy(pDest + stride * pixel_h + i * stride / 2, 
			pSrc + pixel_w * pixel_h + pixel_w * pixel_h / 4 + i * pixel_w / 2,
			pixel_w / 2);
	}
	for (i = 0; i < pixel_h / 2; i++) {
		memcpy(pDest + stride * pixel_h + stride * pixel_h / 4 + i * stride / 2,
			pSrc + pixel_w * pixel_h + i * pixel_w / 2, 
			pixel_w / 2);
	}
#endif
...
extern "C" EXPORT_API IDirect3DSurface9* Init(HWND hwnd,
	...
	unsigned long lHeight)
{
	if (fp == NULL) {
		...
		fp = fopen("../test_yuyv422_320x180.yuv", "rb+");
#endif
	}
	//if (fp == NULL) {
	//	printf("Cannot open this file.\n");
	...
	lRet = m_pDirect3DDevice->CreateRenderTarget(
		lWidth, lHeight,
		D3DFMT_X8R8G8B8,
		D3DMULTISAMPLE_NONE, 0,
		true, // lockable (true for compatibility with Windows XP.  False is preferred for Windows Vista or later)
	...
	//m_pDirect3DDevice->SetRenderTarget(0, m_pDirect3DSurfaceRender);

	return m_pDirect3DSurfaceRender;
}



extern "C" EXPORT_API int  Play(HWND hwnd)
{
